...
// @description:ja 右クリックメニューやショートカットでブラウザ履歴の最初に戻る
// @description:en A UserScript that navigates back to the very first page of the browser history in one go.
// @author         ねおん
// @match          *://*/*
...
(function() {
    ...
    // メッセージ受信用の設定（親ウィンドウ側で動くわ）
    window.addEventListener('message', (event) => {
        // セキュリティのため、自分の関連ドメインからのメッセージかチェックしてもいいけど、
        // 履歴を戻すだけならリスクは低いから、実行しちゃうわね。
        if (event.data && event.data.type === 'HGF_GO_BACK') {
            if (DEBUG) {
                console.log('[HGF] 親ウィンドウでリクエストを受信しました');
            }
    ...
    function goBackToHistoryStart() {
        if (DEBUG) {
            console.log('--- [HGF Debug] 処理開始 ---');
        }

        let isDirectAccessPossible = false;
        try {
            // 親に直接触れるかチェック
            if (window.top && window.top.history) {
                isDirectAccessPossible = true;
            }
        } catch (e) {
            isDirectAccessPossible = false;
        }

        // 親に触れない（iframe内かつ別ドメイン）なら、親にメッセージを送る
        if (!isDirectAccessPossible && window.self !== window.top) {
            if (DEBUG) {
                console.log('[HGF] 親に直接触れないため、メッセージを送信します');
            }
            window.top.postMessage({ type: 'HGF_GO_BACK', }, '*');
            return;
        }
        ...
        try {
            ...
            target = window.top.history;
        } catch (e) {
            console.warn('Target: window.top にアクセス拒否されたよ (クロスドメイン制限)', e);
            target = window.history;
        }

        if (DEBUG) {
            console.log('History Length:', target.length);
        ...
        try {
            if (DEBUG) {
                console.log('Marker設定開始:', uniqueId);
            }
            target.replaceState({ ...target.state, 'hgf_marker': uniqueId, }, '');
        } catch (e) {
            // クロスドメイン制限で書き換えられない場合
            console.error('Marker設定失敗:', e);
            showToast('このページでは履歴操作が制限されているみたい...', false);
            return;
        }
        showToast('履歴を探索中だよ...', null);

        let delta = 1 - target.length;
        if (DEBUG) {
            console.log('計算された Delta:', delta);
        }
        ...
        function checkAndGo() {
            ...
            try {
                ...
                if (DEBUG) {
                    console.log('Loop - Current State:', currentState);
                }
            } catch (e) {
                console.warn('Loop - State取得失敗:', e);
                currentState = null;
            }

            const isSuccessful = !currentState || currentState.hgf_marker !== uniqueId;
            if (DEBUG) {
                console.log('Loop - 成功判定:', isSuccessful, '(Marker:', uniqueId, ')');
            }

            // 履歴の最初に戻れた場合
            if (isSuccessful) {
                if (DEBUG) {
                    console.log('--- [HGF Debug] 成功！終了します ---');
                }
            ...
            if (delta >= 0) {
                // 終了前にマーカーをきれいに削除するよ！
                if (DEBUG) {
                    console.log('--- [HGF Debug] 限界到達 (delta>=0) ---');
                }
                try {
                    target.replaceState({ ...currentState, 'hgf_marker': undefined, }, '');
                } catch (e) {
                    // console.error('マーカーの削除に失敗したよ！', e);
                }
                showToast('既に最初のページにいるよ！', true);
                return;
            }
            ...
            if (DEBUG) {
                console.log('履歴移動実行: history.go(', delta, ')');
            }
            target.go(delta);
            delta++;
    ...
    function normalizeShortcutString(s) {
        ...
        for (const p of parts) {
            if (['ctrl','control',].includes(p)) {
                mods.add('Ctrl');
            } else if (['alt','option',].includes(p)) {
                mods.add('Alt');
            } else if (['shift',].includes(p)) {
                mods.add('Shift');
            } else if (['meta','cmd','command','⌘',].includes(p)) {
                mods.add('Meta');
            } else {
                main = p;
            }
        ...
        if (/^key[a-z]$/i.test(main)) {
            main = main.slice(3);
        }
        if (/^digit[0-9]$/i.test(main)) {
            main = main.slice(5);
        }
        if (/^[a-z]$/.test(main)) {
            main = main.toUpperCase();
        }
        if (/^f([1-9]|1[0-2])$/i.test(main)) {
            main = main.toUpperCase();
        }
    ...
    function eventMatchesShortcut(e, shortcut) {
        ...
        if (need.Ctrl !== e.ctrlKey) {
            return false;
        }
        if (need.Alt !== e.altKey) {
            return false;
        }
        if (need.Shift !== e.shiftKey) {
            return false;
        }
        if (need.Meta !== e.metaKey) {
            return false;
        }

        let pressed = '';
        if (e.code.startsWith('Key')) {
            pressed = e.code.slice(3).toUpperCase();
        } else if (e.code.startsWith('Digit')) {
            pressed = e.code.slice(5);
        } else {
            pressed = e.key.length===1?e.key.toUpperCase():e.key;
        }
    ...
    function ensureStyle() {
        ...
        .hgf-button {
            padding: 10px 20px;
            border:none;border-radius:6px;font-weight:bold;cursor:pointer;transition:all 0.2s ease;background:#007bff;color:white;
        }
        .hgf-button:hover {
            background: #0056b3;
        }
    ...
    function openSettings() {
        ...
        panel.className='hgf-panel';
        const closeBtn = document.createElement('span');
        closeBtn.className='hgf-close';
        closeBtn.textContent='×';
        closeBtn.title='閉じる';
        closeBtn.addEventListener('click',()=>document.body.removeChild(overlay));
        const title = document.createElement('div');
        title.className='hgf-title';
        title.textContent='設定';
        title.appendChild(closeBtn);

        const section = document.createElement('div');
        section.className='hgf-section';
        const label = document.createElement('div');
        label.className='hgf-label';
        label.textContent='ショートカットキー';
        const input = document.createElement('input');
        input.type='text';
        input.className='hgf-input';
        input.placeholder='例: Ctrl+Shift+H';
        input.readOnly=true;
        ...
        bottom.className='hgf-bottom';
        const version=document.createElement('div');
        version.className='hgf-version';
        version.textContent='('+SCRIPT_VERSION+')';
        const saveBtn=document.createElement('button');
        saveBtn.className='hgf-button';
        ...
        panel.appendChild(section); panel.appendChild(bottom);
    ...
    function addContextMenu() {
        ...
        if(settingsId) {
            GM_unregisterMenuCommand(settingsId);
        }
